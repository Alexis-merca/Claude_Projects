...
/** Écart au tracé calculé : une ligne de la table `fleches`. */
export interface EcartFleche {
  id: string;
  de_id: string;
  vers_id: string;
...
export interface FlecheTracee {
  de: number;
  vers: number;
  /** Identifiant en base pour une flèche dessinée, `null` si calculée. */
  id: string | null;
  nature: string;
  manuelle: boolean;
  retour: boolean;
...
export interface OptionsFlux {
  ...
  /** Commandes d'édition émises. Absent : toutes. Présent : seules celles à
      ...
      `colonnes` et `fleches` sont NOUVELLES, donc à opt-in STRICT : absentes,
      elles ne rendent rien, et la sortie reste celle du mono-fichier. */
  commandes?: {
    texte?: boolean;
    phases?: boolean;
    deplacement?: boolean;
...
/** Options du TRACÉ. `fleches` porte les écarts au calcul ; absent, le tracé
...
export function flechesEffectives(
  etapes: EtapeFlux[],
  ecarts?: EcartFleche[] | null,
): FlecheTracee[];
